...
def test_versions_and_prerelease():
    settings=re.search(r"APP_VERSION = ['\"]([^'\"]+)", (ROOT/'src/settings.py').read_text()).group(1)
    package=re.search(r'__version__ = "([^"]+)', (ROOT/'src/__init__.py').read_text()).group(1)
    installer=re.search(r'AppVersion=(.+)', (ROOT/'installer/PCAutoSpec.iss').read_text()).group(1)
...
def test_release_gates_precede_packaging():
    local=(ROOT/'scripts/build_windows_release.ps1').read_text()
    hosted=(ROOT/'.github/workflows/windows-release.yml').read_text()
